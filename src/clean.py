...
def remove_duplicates(data: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"{duplicated_mask.sum()} duplicated rows dropped.")
    return data
...
def remove_unsafe_ratings(data: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"{unsafe_ratings.sum()} unsafe ratings dropped.")
    return data
...
def remove_low_members_anime(data: pd.DataFrame, quantile: float=0.25) -> pd.DataFrame:
    ...
    print(f"{data[data['members'] <= members_threshold].shape[0]} anime removed below {members_threshold} members.")
...
def remove_unlabeled_anime(data: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    print(f'{data["score"].isnull().sum()} missing labels removed from sample.')
